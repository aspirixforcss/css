...
const LoginScreen = ({ onLogin }) => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-bgLight dark:bg-bgDark p-4 relative overflow-hidden transition-colors">
          {showWarning && (
              <div className="absolute top-4 left-4 right-4 md:left-auto md:right-4 md:w-96 bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-800 text-blue-800 dark:text-blue-300 p-4 rounded-2xl shadow-lg z-50 flex items-start gap-3 animate-fade-in">
                  <i className="fa-solid fa-laptop text-xl mt-0.5"></i>
                  <div className="flex-1">
                      <h4 className="font-bold mb-1">Desktop Recommended</h4>
                      <p className="text-sm font-medium opacity-90">This app is best used on a laptop or desktop computer for optimal layout and study features.</p>
                  </div>
                  <button onClick={() => setShowWarning(false)} className="text-blue-500 hover:text-blue-700"><i className="fa-solid fa-xmark"></i></button>
              </div>
          )}
...
const OnboardingWizard = ({ onComplete }) => {
    ...
    return (
        <div className="fixed inset-0 bg-bgLight dark:bg-bgDark overflow-y-auto p-4 md:p-8 transition-colors z-50 font-sans">
            <div className="max-w-5xl mx-auto animate-fade-in pb-32">
                <div className="text-center mb-10 mt-6">
                    <h2 className="text-4xl font-extrabold text-slate-800 dark:text-white mb-4">Set Up Your Profile</h2>
                    <p className="text-slate-500 font-medium max-w-xl mx-auto">Let's customize your CSS journey. Please select your target exam year and choose 600 marks worth of optional subjects according to FPSC rules.</p>
                </div>

                <div className="bg-white dark:bg-surfaceDark p-6 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-700 mb-8">
                    <h3 className="font-bold text-lg text-slate-800 dark:text-white border-b border-slate-100 dark:border-slate-700 pb-4 mb-6">Target Exam Year</h3>
                ...
                <div className="bg-white dark:bg-surfaceDark p-6 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-700 mb-8 sticky top-4 z-20">
                    <div className="flex justify-between items-center mb-4">
                        <h3 className="font-bold text-lg text-slate-800 dark:text-white">Optional Subjects <span className="text-sm font-medium text-slate-500 ml-2">(600 Marks Required)</span></h3>
                        <div className="text-2xl font-black text-primary">{totalMarks}<span className="text-slate-400 text-lg">/600</span></div>
                    </div>
                    <div className="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-3 mb-2 overflow-hidden">
                        <div className="bg-gradient-to-r from-primary to-green-400 h-3 rounded-full transition-all duration-500 ease-out" style={{ width: `${progressPercentage}%` }}></div>
                    </div>
                    {errorMessage && <p className="text-red-500 text-sm font-bold animate-shake mt-2 bg-red-50 dark:bg-red-900/20 p-3 rounded-lg border border-red-200 dark:border-red-800/30"><i className="fa-solid fa-circle-exclamation mr-1"></i> {errorMessage}</p>}
                </div>
                ...
                <div className="space-y-6">
                    <div className="bg-white dark:bg-surfaceDark p-6 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-700">
                        <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-700 pb-4 mb-6">
                            <h3 className="font-bold text-lg text-slate-800 dark:text-white">Compulsory Subjects <span className="text-sm font-medium text-slate-500 ml-2">(600 Marks)</span></h3>
                            <i className="fa-solid fa-lock text-slate-300"></i>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                            {compulsorySubjects.map(sub => (
                                <div key={sub.id} className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-200 dark:border-slate-700 flex justify-between items-center opacity-80 cursor-not-allowed">
                                    <span className="font-bold text-slate-700 dark:text-slate-300">{sub.name}</span>
                                    <span className="text-xs font-black bg-slate-200 dark:bg-slate-700 text-slate-500 px-2 py-1 rounded">{sub.marks}</span>
                                </div>
                            ))}
                        </div>
                    </div>
                                ...
                                return (
                                    <div key={groupNum} className="relative pl-5 border-l-2 border-slate-100 dark:border-slate-700">
                                        <div className="flex justify-between items-end mb-4">
                                            <div>
                                                <h4 className="font-extrabold text-slate-700 dark:text-slate-300">Group {groupNum}</h4>
                                                <p className="text-xs font-bold text-primary">{groupRule}</p>
                                            </div>
                                        </div>
                                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                            {groupSubjects.map(sub => {
                                                ...
                                                return (
                                                    <div 
                                                        ...
                                                        className={`p-4 rounded-xl border-2 cursor-pointer transition-all flex justify-between items-center select-none active:scale-95 ${isSelected ? 'border-primary bg-primary/10 shadow-sm shadow-primary/10' : 'border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 hover:border-primary/40'}`}
                                                    >
                                                        <div className="flex items-center gap-3">
                                                            <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${isSelected ? 'border-primary bg-primary' : 'border-slate-300 dark:border-slate-600'}`}>
                                                                {isSelected && <i className="fa-solid fa-check text-white text-[10px]"></i>}
                                                            </div>
                                                            <span className={`font-bold ${isSelected ? 'text-primary' : 'text-slate-700 dark:text-slate-300'}`}>{sub.name}</span>
                                                        </div>
                                                        <span className={`text-xs font-black px-2 py-1 rounded ${isSelected ? 'bg-primary/20 text-primary' : 'bg-slate-100 dark:bg-slate-700 text-slate-500'}`}>{sub.marks}</span>
                                                    </div>
                                                );
                                            })}
                                        </div>
                ...
                <div className="fixed bottom-0 left-0 right-0 p-4 bg-white/80 dark:bg-surfaceDark/80 backdrop-blur-md border-t border-slate-200 dark:border-slate-700 z-30 flex justify-center shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
                    <button 
                        disabled={!isComplete}
                        onClick={() => onComplete(selected, targetYear)}
                        className={`max-w-md w-full py-4 px-8 rounded-2xl font-black text-lg transition-all shadow-lg flex justify-center items-center gap-3 ${isComplete ? 'bg-primary text-white hover:-translate-y-1 hover:shadow-primary/40' : 'bg-slate-200 dark:bg-slate-800 text-slate-400 cursor-not-allowed'}`}
                    >
                        {isComplete ? (
                            <>Complete Setup <i className="fa-solid fa-arrow-right"></i></>
                        ) : (
                            `Select ${600 - totalMarks} more marks`
                        )}
                    </button>
                </div>
            </div>
...
const Dashboard = ({ setCurrentView, targetYear, selectedSubjects, completedTopics, openTimer, dailyStreak, user }) => {
    ...
    return (
        <div className="max-w-6xl mx-auto p-4 md:p-8 animate-fade-in pb-32 w-full font-sans">
            ...
            {/* MPT Mock Test Banner */}
            <div className="bg-gradient-to-r from-slate-900 to-slate-800 rounded-2xl p-4 md:p-5 mb-8 text-white flex flex-row items-center justify-between shadow-md relative overflow-hidden">
                <div className="absolute right-0 top-0 w-32 h-32 bg-primary/20 blur-2xl rounded-full -mr-10 -mt-10"></div>
                <div className="relative z-10 flex items-center gap-4">
                    <div className="px-2 py-1 bg-white/20 text-white font-bold text-[10px] rounded mb-1 backdrop-blur-md">PRO</div>
                    <h2 className="text-xl font-bold flex items-center gap-2">
                        <i className="fa-solid fa-graduation-cap text-primary"></i> MPT Mock
                    </h2>
                </div>
                <div className="relative z-10">
                    <button onClick={() => setCurrentView('mptmock')} className="px-5 py-2 bg-primary hover:bg-primaryDark text-white font-bold rounded-lg shadow-sm transition-all hover:scale-105 flex items-center justify-center gap-2 text-sm">
...
const AboutView = () => (
    <div className="max-w-4xl mx-auto p-6 md:p-10 bg-white dark:bg-surfaceDark rounded-3xl shadow-xl mt-10">
        ...
        <p className="text-lg text-slate-700 dark:text-slate-300 leading-relaxed mb-4">
            Welcome to Aspirix! This platform is driven by dedicated CSS Mentors who work tirelessly to improve this app and provide the best resources for aspirants. 
        </p>
        <p className="text-lg text-slate-700 dark:text-slate-300 leading-relaxed mb-4">
            ...
            <div className="w-full bg-slate-50 dark:bg-slate-800/50 p-5 sm:p-6 rounded-2xl border border-slate-200 dark:border-slate-700 text-left mb-6">
                ...
                <div className="space-y-2 sm:space-y-3 mb-5 bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
                    ...
                    <p className="flex justify-between pb-2">
                        ...
                        <span className="font-bold text-slate-800 dark:text-white text-sm">Rs. 199</span>
                    </p>
                    <p className="flex justify-between">
                        <span className="text-slate-500 text-sm">1 Year Plan:</span>
                        <span className="font-bold text-slate-800 dark:text-white text-sm">Rs. 1500</span>
                    </p>
                </div>
...
const SettingsModal = ({ isOpen, onClose, onChangeSubjects, onSignOut, isPro, plan, proExpiresAt, onRenew, onSupport }) => {
    if (!isOpen) return null;
            ...
            <div className="bg-white dark:bg-surfaceDark w-full max-w-sm rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-700 overflow-hidden relative">
                ...
                <div className="p-6 space-y-4">
                    ...
                    <div className="space-y-2">
                        ...
                        <button onClick={() => { onClose(); onSupport(); }} className="w-full flex items-center justify-between p-4 rounded-xl bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-colors font-bold text-left group">
                            <span className="flex items-center"><i className="fa-solid fa-headset w-6 text-blue-400 group-hover:text-blue-500 transition-colors"></i> Contact Support / Report Bug</span>
                            <i className="fa-solid fa-chevron-right text-sm opacity-50 group-hover:translate-x-1 transition-transform"></i>
                        </button>
...
if (currentView === 'upgrade') return <PremiumUpgradeView onUpgrade={handleUpgrade} user={user} />;
