...
const idx = code.indexOf('liveVocab.length} words</strong> loaded!</div></>'); 
const insertionPt = code.indexOf('</div>', idx) + 6; 
...
code = code.substring(0, insertionPt) + replace + code.substring(insertionPt);
fs.writeFileSync('scratch/VocabFlashcards.js', code);
console.log('Patched via slice');
